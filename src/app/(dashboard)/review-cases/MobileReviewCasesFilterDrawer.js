...
export function MobileReviewCasesFilterDrawer({
  open,
  onOpenChange,
  totalCount,
  isPending,
  ...
  return (
    <Drawer open={open} onOpenChange={onOpenChange} shouldScaleBackground={false}>
      <DrawerContent className="lg:hidden max-h-[96dvh] p-0 gap-0 flex flex-col">
        ...
        <div className="flex flex-1 min-h-0 flex-col overflow-y-auto overscroll-contain px-4 py-4 pb-8">
          <ReviewCasesFilterPanel
            layout="stacked"
            onFilterApplied={() => onOpenChange(false)}
            {...filterPanelProps}
          />
        </div>
      </DrawerContent>

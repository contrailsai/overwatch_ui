...
export function FeedContentList({
  ...
  const handleHistogramDayClick = useCallback(
    (isoDay) => {
      ...
      updateQueryParams({
        published_from: new Date(`${isoDay}T00:00:00.000Z`).toISOString(),
        published_to: new Date(`${isoDay}T23:59:59.999Z`).toISOString(),
        original_date_from: null,
        original_date_to: null,
      })
    },
  ...
  const activePublishDay = useMemo(() => {
    const from = initialFilters.published_from || initialFilters.original_date_from
    const to = initialFilters.published_to || initialFilters.original_date_to
  ...
  }, [initialFilters.published_from, initialFilters.published_to, initialFilters.original_date_from, initialFilters.original_date_to])

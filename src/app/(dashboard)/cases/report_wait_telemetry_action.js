...
/**
 * Persists a single end-of-session summary for the client report wait flow.
 * - **Tempo:** span `telemetry.report_wait.flush` (child of the incoming server-action trace when present).
 * - **Loki:** one JSON line on stdout (typical Vercel → log shipper → Loki pipelines).
 * - **Mimir:** counter `report_wait_session_outcomes_total` by outcome / format.
 *
 * Enable in local dev with `NEXT_PUBLIC_REPORT_WAIT_TELEMETRY=1` (off by default in dev to limit noise).
 */
export async function flushReportWaitTelemetry(summary) {
  ...
  return runInSpan('telemetry.report_wait.flush', async (span) => {
    ...
    // Single JSON line for Loki / log aggregators (search: report_wait_telemetry)
    console.info(
      JSON.stringify({
...
function sanitizeSummary(s) {
  ...
  return {
    ...
    completion_source: raw.completion_source ? clip(String(raw.completion_source), 16) : null,

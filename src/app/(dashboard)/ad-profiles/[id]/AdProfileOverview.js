...
function AdProfileAvatar({ profile }) {
  const src = profile.metadata?.profile_pic
  const initial = (profile.display_name || profile.page_name || '?').charAt(0).toUpperCase()
  if (src) {
    return (
...
export function AdProfileOverview({ profile, project, analytics, ads, domains, range }) {
  ...
  const pageCategories = meta.page_categories || []
  const pageIsDeleted = meta.page_is_deleted ?? false
  const pageLikeCount = meta.page_like_count ?? meta.follower_count ?? 0
  const adsCount = profile.ads_count ?? meta.media_count ?? 0
  const displayPlatform = profile.platform === 'meta' ? 'facebook' : profile.platform
  ...
  const infoCard = (
    <section className="bg-white border border-slate-200 rounded-xl p-5">
      <div className="flex items-start gap-4">
        <AdProfileAvatar profile={profile} />
        <div className="min-w-0 flex-1 space-y-3">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h2 className="text-lg font-bold text-slate-900 tracking-tight truncate">
                {profile.display_name || profile.page_name}
              </h2>
              {profile.is_verified ? <BadgeCheck className="w-4 h-4 text-blue-500 shrink-0" /> : null}
              {pageIsDeleted ? (
                <Badge
                  variant="outline"
                  className="h-5 px-1.5 text-[9px] font-bold bg-rose-50 text-rose-700 border-rose-200 uppercase tracking-wider gap-1"
                >
            ...
            <div className="flex items-center gap-2 mt-0.5 text-sm flex-wrap">
              {profile.platform_page_id ? (
                <span className="font-medium text-slate-500 font-mono text-xs">
                  {profile.platform_page_id}
                </span>
              ) : null}
              {profile.platform_page_id ? <span className="text-slate-300">·</span> : null}
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {Array.isArray(pageCategories) && pageCategories.length > 0
              ? pageCategories.map((cat, idx) => {
                  ...
                  return (
                    <div
                      key={`${label}-${idx}`}
                      className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-slate-100 text-slate-600 text-xs font-medium border border-slate-200/60"
                    >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 pt-3 border-t border-slate-100">
            <div>
              <p className="text-lg font-bold text-slate-900 tracking-tight tabular-nums">
                {Number(pageLikeCount || 0).toLocaleString()}
              </p>
            ...
            <div>
              <p className="text-lg font-bold text-slate-900 tracking-tight tabular-nums">
                {Number(adsCount || 0).toLocaleString()}
              </p>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">
                Ads
              </p>
            ...
            <div>
              <p className="text-lg font-bold text-slate-900 tracking-tight tabular-nums">
                {Number(domains?.length || 0).toLocaleString()}
              </p>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">
                Domains
              </p>
          ...
          <p className="text-xs text-slate-400 tabular-nums">
            {inRangeCount.toLocaleString()} ads in selected range
          </p>
        </div>
      </div>
    </section>
    ...
    <div className="space-y-3">
      <div className="bg-white border border-slate-200 rounded-xl p-3">
        ...
        {timelineData.length === 0 || timelineData.every((d) => d.count === 0) ? (
          <p className="text-xs text-slate-400 py-6 text-center">No ads in this range</p>
        ) : (
          <div className="h-28">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={timelineData} margin={{ left: 0, right: 4, top: 4, bottom: 0 }}>
                <defs>
                  <linearGradient id="adProfileAdsFill" x1="0" y1="0" x2="0" y2="1">
                ...
                <Tooltip formatter={(value) => [value, 'Ads']} labelFormatter={(label) => label} />
                <Area
                  type="monotone"
                  dataKey="count"
                  stroke="var(--primary)"
                  strokeWidth={2}
                  fill="url(#adProfileAdsFill)"
                />
              </AreaChart>
            </ResponsiveContainer>
          </div>
  ...
  const domainsBlock = (
    <section>
      ...
      {domains.length === 0 ? (
        <div className="bg-white border border-slate-200 rounded-xl px-5 py-8 text-center text-sm text-slate-400">
          No reviewed domains linked from this advertiser’s ads
        </div>
      ) : (
        <AdProfileLinkedDomains domains={domains} hrefBase="/domains" />
      )}
    </section>
  )
  ...
  const titleName = profile.display_name || profile.page_name || 'Ad Profile'
    ...
    <main className="flex-1 flex flex-col h-full overflow-hidden bg-slate-50">
      ...
      <div className="shrink-0 border-b border-slate-200 bg-white px-4 sm:px-6 lg:px-8 py-3 flex flex-col sm:flex-row sm:items-center gap-3 justify-between">
        <div className="min-w-0">
          ...
          {profile.platform_page_id ? (
            <p className="text-sm text-slate-500 font-medium font-mono truncate">{profile.platform_page_id}</p>
          ) : null}
        </div>
        <DateRangeControls
